...
(async q => {
    ...
    const addDescripton = (isDescriptionAvilable, isJaTranslationAvailable) => {
        ...
        const text = isDescriptionAvilable
            ? "にこのMODの説明があります。"
            : "にこのMODの説明はありませんでした。";
        const text2 = isJaTranslationAvailable ? "また、日本語化ファイルがあります。" : "";
        const html = `${divider}<p>${link}${text}<br>${text2}</p>`;
    ...
    addDescripton(isDescriptionAvilable(doc), isJaTranslationAvailable(doc));
